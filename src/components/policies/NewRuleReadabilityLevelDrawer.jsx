...
const NewRuleReadabilityLevelDrawer = ({
  open,
  onClose,
  onSave,
  initialData,
  ...
  useEffect(() => {
    if (open && initialData) {
      ...
      setSearchFor(
        initialData?.searchFor !== undefined ? initialData.searchFor : "",
      );
      setReadabilityScore(
        initialData?.readabilityScore !== undefined
          ? initialData.readabilityScore
          : "",
      );
      ...
      >
        <div className="border-bottom border-secondary border-opacity-25 px-4 py-3 flex-shrink-0">
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              <h5
                id="new-rule-readability-level-title"
                className="mb-0 fw-semibold text-body"
              >
                New rule - Readability level
              </h5>
              <p className="text-muted fs-13 mb-0 mt-1">
                Search for text that is in compliance with school degree
              </p>
            </div>
          </div>
        </div>
