...
const NewRulePageHtmlDrawer = ({ open, onClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      setSelectors(
        initialData.selectors?.length > 0
          ? initialData.selectors
          : [{ type: "Limit search", value: "" }],
      );
    } else {
      // Reset for new rule
        ...
        <div className="flex-grow-1 overflow-auto p-4">
          ...
          <div className="mb-4">
            ...
            <div className="d-flex align-items-start gap-2 flex-wrap">
              <div className="form-check">
                ...
                />
                <label className="form-check-label fs-13" htmlFor="containing">
                  Containing
                </label>
              </div>
              <div className="form-check">
          ...
          <div className="mb-4">
            ...
            <p className="text-muted fs-12 mb-3">
              Add CSS selectors to either limit the search within those values
              or exclude the snippets with the given inputs. You can add as many
              selectors as you wish.
            </p>
            <div className="bg-light bg-opacity-50 rounded-2 p-3 mb-3">
              <p className="fs-12 mb-1">
                <span className="text-muted">Example 1:</span>{" "}
                <span className="text-danger">.class-name1</span>
              </p>
              <p className="fs-12 mb-1">
                <span className="text-muted">Example 2:</span>{" "}
                <span className="text-danger">#idname</span>
              </p>
              <p className="fs-12 mb-1">
                <span className="text-muted">Example 3:</span>{" "}
                <span className="text-danger">div.container#main</span>
              </p>
              <p className="fs-12 mb-0">
                <span className="text-muted">Example 4:</span>{" "}
                <span className="text-danger">a[href^="https"]</span>
              </p>
            </div>
            {selectors.map((row, index) => (
              <div
                key={index}
                className="d-flex align-items-center gap-2 mb-2 flex-nowrap"
              >
                <select
                  ...
                  value={row.type}
                  onChange={(e) =>
                    updateSelector(index, "type", e.target.value)
                  }
                >
