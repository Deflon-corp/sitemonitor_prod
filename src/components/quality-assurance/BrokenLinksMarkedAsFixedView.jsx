...
export default function BrokenLinksMarkedAsFixedView({
  onOpenContentDrawer,
  ...
  const handleAction = async (row, actionType) => {
    ...
    try {
      ...
      if (actionType === "reopen") {
        res = await patchQaLinkStatusApi(domainId, {
          href: row.url,
          isFixed: false,
        });
      ...
      if (res?.success) {
        toast.success(
          actionType === "reopen"
            ? "Link reopened successfully"
            : "Link ignored successfully",
        );
        fetchFixedLinks();
      } else {
    ...
    <div className="d-flex flex-column h-100">
      <div className="mb-3">
        <p className="text-muted fs-13 mb-0">
          {sortedRows.length} marked as fixed
        </p>
      </div>
                ...
                <tr>
                  <td colSpan="7" className="text-center py-5">
                    <span className="text-muted">
                      No marked as fixed links found.
                    </span>
                  </td>
                </tr>
              ) : (
