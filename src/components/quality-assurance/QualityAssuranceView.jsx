...
const LINK_VIEW_KEYS = [
  "links",
  "content-broken-links",
  "broken-links",
  "broken-images",
  "broken-links-sitemap",
];
const SUMMARY_LINKS_VIEW_KEYS = [
  "summary-broken-links",
  "summary-broken-images",
];
const SPELLCHECK_VIEW_KEYS = [
  "spellcheck",
  "spellcheck-summary",
  "spellcheck-pages",
  "spellcheck-misspellings",
  "spellcheck-potential",
  "spellcheck-dictionary",
  "spellcheck-ignored",
];
const SUMMARY_SPELLCHECK_VIEW_KEYS = [
  "summary-potential-misspellings",
  "summary-misspellings",
];
const READABILITY_VIEW_KEYS = [
  "readability",
  "readability-summary",
  "readability-checker",
];
...
const LINKS_SUB_NAV = [
  {
    key: "content-broken-links",
    label: "Pages with broken links",
    icon: "isax-document-text",
    href: "/domain/quality-assurance?view=content-broken-links",
  },
  {
    key: "broken-links",
    label: "Broken links",
    icon: "isax-link-2",
    href: "/domain/quality-assurance?view=broken-links",
  },
  {
    key: "broken-images",
    label: "Broken images",
    icon: "isax-image",
    href: "/domain/quality-assurance?view=broken-images",
  },
  {
    key: "broken-links-sitemap",
    label: "Broken links on sitemap",
    icon: "isax-menu",
    href: "/domain/quality-assurance?view=broken-links-sitemap",
  },
];

const SPELLCHECK_SUB_NAV = [
  {
    key: "spellcheck-summary",
    label: "Overview",
    icon: "isax-home-2",
    href: "/domain/quality-assurance?view=spellcheck-summary",
  },
  {
    key: "spellcheck-pages",
    label: "Pages with misspellings",
    icon: "isax-document-text",
    href: "/domain/quality-assurance?view=spellcheck-pages",
  },
  {
    key: "spellcheck-misspellings",
    label: "Misspellings",
    icon: "isax-edit-2",
    href: "/domain/quality-assurance?view=spellcheck-misspellings",
  },
  {
    key: "spellcheck-potential",
    label: "Possible misspellings",
    icon: "isax-edit-2",
    href: "/domain/quality-assurance?view=spellcheck-potential",
  },
];

const READABILITY_SUB_NAV = [
  {
    key: "readability-summary",
    label: "Overview",
    icon: "isax-home-2",
    href: "/domain/quality-assurance?view=readability-summary",
  },
  {
    key: "readability-checker",
    label: "By page",
    icon: "isax-discovery",
    href: "/domain/quality-assurance?view=readability-checker",
  },
];
...
const QA_NAV = [
  {
    ...
    label: "Overview",
    icon: "isax-home-2",
    href: "/domain/quality-assurance?view=summary",
  },
  {
    key: "qa-errors",
    label: "Pages with issues",
    icon: "isax-document-copy",
    href: "/domain/quality-assurance?view=qa-errors",
  },
  {
    key: "links",
    label: "Links",
...
function buildQASummaryExportRows(qa) {
  ...
  return [
    {
      category: "Unique broken links",
      value: String(qa.uniqueBrokenLinks ?? 0),
      detail: `Affects ${qa.pagesWithBrokenLinks ?? 0} pages`,
    },
    {
      category: "Possible misspellings (unique words)",
      value: String(
        qa.uniquePotentialMisspellings ??
          qa.topPotentialMisspellings?.length ??
          0,
      ),
    ...
    },
    {
      ...
      value: String(qa.uniqueBrokenImages ?? 0),
      detail: `Affects ${qa.pagesWithBrokenImages ?? 0} pages`,
    },
    {
    ...
    },
    {
      ...
      value: `${qa.qaCompliancePercent ?? 0}%`,
      detail: "Pages compliant with all QA checks",
    },
    {
      category: "Industry average",
      value: `${INDUSTRY_AVERAGE_PERCENT}%`,
      detail: "Industry benchmark",
    },
    {
      category: "Total QA issues",
      value: String(qa.totalQaIssues ?? 0),
      detail: "Total count of QA issues",
    },
    {
      category: "Content with issues",
      value: String(qa.contentWithQaErrors ?? 0),
      detail: "Pages with at least one issue",
    },
    {
  ...
  return (
    <div className="position-relative d-inline-flex flex-column align-items-center">
      ...
      <span className="d-flex align-items-center gap-1 mt-2 fs-13 text-muted">
        ...
        {showInfo && (
          <i
            className="isax isax-info-circle fs-14 opacity-75"
            aria-hidden="true"
...
export default function QualityAssuranceView() {
  ...
  const qaSummaryExportRows = useMemo(
    () => buildQASummaryExportRows(qaSummary),
    [qaSummary],
  );
  ...
  const exportQASummaryCSV = useCallback(() => {
    ...
    const body = qaSummaryExportRows
      .map(
        (r) =>
          `"${r.category.replace(/"/g, '""')}","${r.value.replace(/"/g, '""')}","${(r.detail ?? "").replace(/"/g, '""')}"`,
      )
      .join("\n");
  ...
  const exportQASummaryExcel = useCallback(async () => {
    ...
    const rows = qaSummaryExportRows.map((r) => ({
      Category: r.category,
      Value: r.value,
      Detail: r.detail ?? "",
    }));
  ...
  const exportQASummaryPDF = useCallback(async () => {
    ...
    const body = qaSummaryExportRows.map((r) => [
      r.category,
      r.value,
      r.detail ?? "",
    ]);
    ...
    <div>
      ...
      <div className="card mb-4">
        <div className="card-body py-3">
          ...
          <div className="d-flex flex-wrap align-items-center gap-3">
            <nav
              className="d-flex flex-wrap gap-1 gap-md-4 align-items-center flex-grow-1"
              aria-label="Quality Assurance navigation"
            >
              {QA_NAV.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="dropdown">
                      ...
                      <ul
                        ...
                        aria-labelledby={`qa-nav-${item.key}`}
                      >
                        {item.children.map((sub) => {
                          const isSubActive =
                            ...
                            (item.key === "links" &&
                              ...
                              sub.key === "broken-links") ||
                            (item.key === "links" &&
                              currentView === "summary-broken-images" &&
                              sub.key === "broken-images") ||
                            (item.key === "spellcheck" &&
                              currentView ===
                                "summary-potential-misspellings" &&
                              sub.key === "spellcheck-potential") ||
                            (item.key === "spellcheck" &&
                              currentView === "summary-misspellings" &&
                              sub.key === "spellcheck-misspellings");
                          return (
                            <li key={sub.key}>
                              <Link
                                to={sub.href}
      ...
      <div className="min-w-0 p-4 bg-body-tertiary rounded-3 overflow-auto">
        ...
        {showSummaryLoader ? (
          ...
          <React.Fragment>
            {!qaSummary && domainId && !isLoading && (
              <div className="alert alert-info fs-13 mb-3" role="status">
                No QA scan data yet. Click <strong>Run QA scan</strong> to
                analyze this domain.
              </div>
            )}
            {/* Header */}
              ...
              <div className="d-flex flex-wrap align-items-start justify-content-between gap-3">
                <div>
                  <h5 className="mb-1 d-flex align-items-center gap-2 text-body fw-semibold">
                    ...
                    >
                      <i
                        className="isax isax-tick-circle fs-22"
                        aria-hidden="true"
                      ></i>
                    </span>
                    Quality Assurance
                  ...
                  <p
                    ...
                    style={{ maxWidth: 520 }}
                  >
                    Check and fix content issues across your website (e.g.
                    Misspellings, Broken links, Readability and other issues).
                  </p>
                </div>
                ...
                <div className="card border-0 shadow-sm rounded-3 mb-4">
                  <div className="card-body p-4">
                    ...
                    <div className="row g-3">
                        ...
                        >
                          <div className="rounded-3 border border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50 p-3 h-100">
                            <div className="d-flex align-items-center gap-2 mb-2">
                              ...
                              >
                                <i
                                  className="isax isax-link-2 fs-16"
                                  aria-hidden="true"
                                ></i>
                              </span>
                            ...
                            <div className="fs-4 fw-bold text-body">
                              {qaSummary?.uniqueBrokenLinks ?? 0}
                            </div>
                            <span className="fs-12 text-muted">
                              Affects {qaSummary?.pagesWithBrokenLinks ?? 0}{" "}
                              pages
                            </span>
                          </div>
                        </Link>
                      ...
                      <div className="col-6">
                        <Link
                          to="/domain/quality-assurance?view=summary-potential-misspellings"
                          className="text-decoration-none text-body d-block h-100"
                        >
                          <div className="rounded-3 border border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50 p-3 h-100">
                            <div className="d-flex align-items-center gap-2 mb-2">
                              <span
                                className="d-inline-flex align-items-center justify-content-center rounded-2 bg-warning bg-opacity-10 text-warning"
                                style={{ width: 32, height: 32 }}
                              >
                                <i
                                  className="isax isax-text fs-16"
                                  aria-hidden="true"
                                ></i>
                              </span>
                              <span className="fs-13 text-body">
                            ...
                            <span className="fs-12 text-muted">
                              Affects{" "}
                              {qaSummary?.pagesWithPotentialMisspellings ?? 0}{" "}
                              pages
                            </span>
                          </div>
                        </Link>
                      ...
                      <div className="col-6">
                        ...
                        >
                          <div className="rounded-3 border border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50 p-3 h-100">
                            <div className="d-flex align-items-center gap-2 mb-2">
                              <span
                                className="d-inline-flex align-items-center justify-content-center rounded-2 bg-info bg-opacity-10 text-info"
                                style={{ width: 32, height: 32 }}
                              >
                                <i
                                  className="isax isax-image fs-16"
                                  aria-hidden="true"
                                ></i>
                              </span>
                              <span className="fs-13 text-body">
                            ...
                            <div className="fs-4 fw-bold text-body">
                              {qaSummary?.uniqueBrokenImages ?? 0}
                            </div>
                            <span className="fs-12 text-muted">
                              Affects {qaSummary?.pagesWithBrokenImages ?? 0}{" "}
                              pages
                            </span>
                          </div>
                        </Link>
                      ...
                      <div className="col-6">
                        <Link
                          to="/domain/quality-assurance?view=summary-misspellings"
                          className="text-decoration-none text-body d-block h-100"
                        >
                          <div className="rounded-3 border border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50 p-3 h-100">
                            <div className="d-flex align-items-center gap-2 mb-2">
                              <span
                                className="d-inline-flex align-items-center justify-content-center rounded-2 bg-danger bg-opacity-10 text-danger"
                                style={{ width: 32, height: 32 }}
                              >
                                <i
                                  className="isax isax-edit-2 fs-16"
                                  aria-hidden="true"
                                ></i>
                              </span>
                              <span className="fs-13 text-body">
                            ...
                            <div className="fs-4 fw-bold text-body">
                              {qaSummary?.uniqueMisspellings ??
                                qaSummary?.topMisspellings?.length ??
                                0}
                            </div>
                            <span className="fs-12 text-muted">
                              Affects {qaSummary?.pagesWithMisspellings ?? 0}{" "}
                              pages
                            </span>
                          </div>
                        </Link>
                  ...
                  <div className="card-body p-4">
                    ...
                    <div className="d-flex flex-column gap-0">
                      <div className="d-flex align-items-start gap-3 py-3 border-bottom border-secondary border-opacity-25">
                        <span
                          className="d-flex align-items-center justify-content-center rounded-2 bg-danger bg-opacity-10 text-danger flex-shrink-0"
                          style={{ width: 40, height: 40 }}
                        >
                          <i
                            className="isax isax-edit-2 fs-18"
                            aria-hidden="true"
                          ></i>
                        </span>
                        <div className="min-w-0 flex-grow-1">
                          <p className="fs-13 text-muted mb-1">
                            Misspelling affecting the most content
                          </p>
                          ...
                          <span
                            className="d-inline-block rounded-2 bg-primary bg-opacity-15"
                      ...
                      <div className="d-flex align-items-start gap-3 py-3 border-bottom border-secondary border-opacity-25">
                        <span
                          className="d-flex align-items-center justify-content-center rounded-2 bg-warning bg-opacity-10 text-warning flex-shrink-0"
                          style={{ width: 40, height: 40 }}
                        >
                          <i
                            className="isax isax-text fs-18"
                            aria-hidden="true"
                          ></i>
                        </span>
                        <div className="min-w-0 flex-grow-1">
                          <p className="fs-13 text-muted mb-1">
                            Potential misspelling affecting the most content
                          </p>
                          <span className="d-inline-block fs-13 fw-semibold text-body bg-warning bg-opacity-10 rounded-2 px-2 py-1 me-2">
                            {qaSummary?.potentialMisspellingAffectingMostContent ||
                              (qaSummary?.topPotentialMisspellings &&
                                qaSummary.topPotentialMisspellings[0]?.word) ||
                              "drawdown"}
                          </span>
                          <span
                            className="d-inline-block rounded-2 bg-primary bg-opacity-15"
                      ...
                      <div className="d-flex align-items-start gap-3 py-3 border-bottom border-secondary border-opacity-25">
                        ...
                        >
                          <i
                            className="isax isax-link-2 fs-18"
                            aria-hidden="true"
                          ></i>
                        </span>
                        <div className="min-w-0 flex-grow-1">
                          <p className="fs-13 text-muted mb-1">
                            Broken link affecting most content
                          </p>
                          {qaSummary?.brokenLinksAffectingMostContent ? (
                            <a
                              href={qaSummary.brokenLinksAffectingMostContent}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="fs-13 text-body text-decoration-none d-inline-flex align-items-center gap-1 bg-danger bg-opacity-10 border border-danger border-opacity-25 rounded-2 px-2 py-1 text-break"
                            >
                              <ExternalLinkIcon
                                size={12}
                      ...
                      <div className="d-flex align-items-start gap-3 py-3">
                        <span
                          className="d-flex align-items-center justify-content-center rounded-2 bg-info bg-opacity-10 text-info flex-shrink-0"
                          style={{ width: 40, height: 40 }}
                        >
                          <i
                            className="isax isax-image fs-18"
                            aria-hidden="true"
                          ></i>
                        </span>
                        <div className="min-w-0 flex-grow-1">
                          <p className="fs-13 text-muted mb-1">
                            Broken image affecting most content
                          </p>
                          {qaSummary?.brokenImagesAffectingMostContent ? (
                            <a
                              href={qaSummary.brokenImagesAffectingMostContent}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="fs-13 text-body text-decoration-none d-inline-flex align-items-center gap-1 bg-danger bg-opacity-10 border border-danger border-opacity-25 rounded-2 px-2 py-1 text-break"
                            >
                  ...
                  <div className="card-body p-4">
                    ...
                    <div className="d-flex align-items-center gap-3 rounded-3 border border-primary border-opacity-25 bg-white bg-opacity-80 p-3">
                      <span className="fw-semibold text-body">
                        {qaSummary?.mostCommonReadabilityLevel || "—"}
                      </span>
                      <span className="text-muted fs-13">
                        Affects {qaSummary?.readabilityPagesCount ?? 0} pages
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="card-body p-3">
                      ...
                      <div className="col-6">
                        <div className="rounded-3 border border-secondary border-opacity-25 p-2">
                          <p className="fs-13 fw-semibold text-body mb-0 d-flex align-items-center gap-1">
                            Total QA issues
                            <i
                              className="isax isax-info-circle fs-12 text-muted opacity-75"
                              aria-hidden="true"
                              title="Total count of QA issues"
                            />
                          </p>
                          <p className="fs-3 fw-bold text-body mb-0 mt-1">
                            {totalIssues}
                          </p>
                          <span className="d-inline-flex align-items-center gap-1 text-success fs-12">
                            <i
                              className="isax isax-arrow-down-1"
                              aria-hidden="true"
                            />
                            0%
                            <i
                              className="isax isax-arrow-down-1 fs-10 opacity-75"
                              aria-hidden="true"
                            />
                          </span>
                        </div>
                        ...
                        <div className="rounded-3 border border-secondary border-opacity-25 p-2">
                          <p className="fs-13 fw-semibold text-body mb-0 d-flex align-items-center gap-1">
                            ...
                            <i
                              className="isax isax-info-circle fs-12 text-muted opacity-75"
                              aria-hidden="true"
                              title="Pages with at least one issue"
                            />
                          </p>
                          <p className="fs-3 fw-bold text-body mb-0 mt-1">
                            {contentWithIssues}
                    ...
                    <div className="d-flex justify-content-end mt-2 pt-2 border-top border-secondary border-opacity-25">
                      <Link
                        ...
                        className="btn btn-sm btn-link text-primary text-decoration-none d-inline-flex align-items-center gap-1 p-0"
                      >
                        Show history
                        <i
                          className="isax isax-arrow-right-1 fs-14"
                          aria-hidden="true"
                        />
                      </Link>
                    </div>
          ...
          <SummaryCategoryView
            title="Unique broken links"
            viewKey="summary-broken-links"
            defaultQaSubView="broken-links"
            icon="isax-link-2"
          />
        ) : currentView === "summary-potential-misspellings" ? (
          <SummaryCategoryView
            title="Potential misspellings"
            viewKey="summary-potential-misspellings"
          ...
          <SummaryCategoryView
            title="Broken images"
            viewKey="summary-broken-images"
            defaultQaSubView="broken-images"
            icon="isax-image"
          />
        ) : currentView === "summary-misspellings" ? (
          <SummaryCategoryView
            title="Misspellings"
            viewKey="summary-misspellings"
