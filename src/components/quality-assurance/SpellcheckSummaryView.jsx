...
function MisspellingsTrendChart() {
  ...
  const maxY = 1000;
  const uniquePts = "0 175 50 174 100 173 150 172 200 171 250 170 300 169 350 168 400 167";
  const potentialPts = "0 12 50 95 100 178 150 245 200 312 250 378 300 434 350 489 400 545";
  ...
  return (
    <div className="position-relative">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-100" style={{ height: 200 }} preserveAspectRatio="xMidYMid meet">
        <line x1={padding.left} y1={padding.top} x2={padding.left} y2={padding.top + chartH} stroke={gridColor} strokeWidth="1" />
        <line x1={padding.left} y1={padding.top + chartH} x2={padding.left + chartW} y2={padding.top + chartH} stroke={gridColor} strokeWidth="1" />
        {/* Y-axis tick labels (0, 500, 1,000) and horizontal grid lines */}
        {yTicks.map((tick) => {
          const y = scaleY(tick);
          return (
            <g key={tick}>
              <line x1={padding.left} y1={y} x2={padding.left + chartW} y2={y} stroke={gridColor} strokeWidth="1" />
              <text x={padding.left - 6} y={y + 4} textAnchor="end" className="fs-10 text-muted" fill="currentColor" style={{ fontSize: 10 }}>
                {tick === 1000 ? "1,000" : String(tick)}
              </text>
            </g>
          );
        })}
        <path d={uniquePath} fill="none" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <path d={potentialPath} fill="none" stroke="#3b82f6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <text x={padding.left + chartW / 2} y={h - 4} textAnchor="middle" className="fs-10 text-muted" fill="currentColor" style={{ fontSize: 10 }}>
          Dec 09 — Feb 15
        </text>
      </svg>
      <div className="d-flex flex-wrap justify-content-center gap-4 mt-2">
        <span className="d-inline-flex align-items-center gap-1 fs-12 text-muted">
          <span className="rounded" style={{ width: 10, height: 10, backgroundColor: "#f59e0b" }} /> Unique misspellings
        </span>
        <span className="d-inline-flex align-items-center gap-1 fs-12 text-muted">
          <span className="rounded" style={{ width: 10, height: 10, backgroundColor: "#3b82f6" }} /> Potential misspellings
        </span>
      </div>
    </div>
  );
...
export default function SpellcheckSummaryView() {
  ...
  const exportCSV = useCallback(() => {
    const summaryRows = [
      ...
      [],
      ["Word", "Language", "Pages"],
      ...MOST_COMMON_MISSPELLINGS.map((r) => [`"${r.word.replace(/"/g, '""')}"`, `"${r.language.replace(/"/g, '""')}"`, String(r.pages)]),
      [],
      ["Word", "Language", "Pages"],
      ...MOST_COMMON_POTENTIAL.map((r) => [`"${r.word.replace(/"/g, '""')}"`, `"${r.language.replace(/"/g, '""')}"`, String(r.pages)]),
    ];
  ...
  const exportPDF = useCallback(async () => {
    ...
    autoTable(doc, {
      ...
      body: MOST_COMMON_MISSPELLINGS.map((r) => [r.word, r.language, String(r.pages)]),
      startY,
      styles: { fontSize: 9 },
    });
    ...
    autoTable(doc, {
      head: [["Word", "Language", "Pages"]],
      body: MOST_COMMON_POTENTIAL.map((r) => [r.word, r.language, String(r.pages)]),
      startY,
      styles: { fontSize: 9 },
    });
            ...
            <div className="col-lg-5">
              <div className="rounded-3 border border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50 p-3">
                ...
                <div className="d-flex align-items-center justify-content-between py-2 border-bottom-0">
                  <span className="d-inline-flex align-items-center gap-2 text-body fs-13">
                    <i className="isax isax-flag text-primary fs-18" aria-hidden="true" />
                    Most common language
                  </span>
                  <span className="fw-semibold text-body fs-13">{MOST_COMMON_LANGUAGE}</span>
                </div>
              </div>
            </div>
      ...
      <div className="row g-4">
        <div className="col-lg-6">
          <div className="card border border-secondary border-opacity-25 shadow-sm rounded-3 h-100 overflow-hidden">
            <div className="card-body p-4">
              <h6 className="fw-semibold text-body mb-3">Most common misspellings</h6>
        ...
        <div className="col-lg-6">
          <div className="card border border-secondary border-opacity-25 shadow-sm rounded-3 h-100 overflow-hidden">
            <div className="card-body p-4">
              <h6 className="fw-semibold text-body mb-3">Most common potential misspellings</h6>
              <div className="table-responsive">
                <table className="table table-hover table-striped table-borderless align-middle mb-0">
                  <thead>
                    <tr className="border-bottom border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50">
