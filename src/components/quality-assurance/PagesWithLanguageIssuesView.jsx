...
const TABS = [
  {
    key: "incorrect",
    label: "Pages with incorrect language",
    icon: "isax-danger",
  },
...
export default function PagesWithLanguageIssuesView() {
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      if (sortBy === "declaredLanguage")
        return dir * a.declaredLanguage.localeCompare(b.declaredLanguage);
      if (sortBy === "detectedLanguage")
        return dir * a.detectedLanguage.localeCompare(b.detectedLanguage);
      if (sortBy === "multiLanguage")
        return (
          dir *
          (a.multiLanguage === b.multiLanguage ? 0 : a.multiLanguage ? 1 : -1)
        );
  ...
  return (
    <div className="pages-with-language-issues-view d-flex flex-column h-100">
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              <h5 className="mb-0 fw-semibold text-body d-flex align-items-center gap-2">
                ...
                <i
                  className="isax isax-tick-circle text-primary fs-18"
                  aria-hidden="true"
                ></i>
              </h5>
              <p className="text-muted fs-13 mb-0 mt-1">
                <span className="fw-medium text-body">{sortedRows.length}</span>{" "}
                page{sortedRows.length !== 1 ? "s" : ""}
              </p>
            </div>
          </div>
      ...
      <nav
        className="nav nav-tabs border-0 gap-2 gap-md-4 mb-4"
              ...
              <thead>
                <tr className="border-bottom border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50">
                  ...
                  <th className="py-3 text-body fs-13 fw-semibold text-nowrap">
                    <SortBtn column="declaredLanguage">
                      Declared language
                    </SortBtn>
                  </th>
                  <th className="py-3 text-body fs-13 fw-semibold text-nowrap">
                    <SortBtn column="detectedLanguage">
                      Detected language
                    </SortBtn>
                  </th>
                  <th className="py-3 text-body fs-13 fw-semibold text-nowrap">
                ...
                {paginatedRows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="py-3 fs-13 text-body">
                      {row.declaredLanguage}
                    </td>
                    <td className="py-3">
                      <span className="badge rounded-pill bg-primary bg-opacity-10 text-primary fs-13 fw-normal">
                    ...
                    <td className="py-3 fs-13 text-body">
                      {row.multiLanguage ? (
                        ...
                        <i
                          className="isax isax-close-circle text-muted"
                          aria-hidden="true"
                          title="No"
                        ></i>
                      )}
