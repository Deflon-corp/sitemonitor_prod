...
export default function UserList() {
  ...
  const handleDeleteUser = async (user) => {
    const confirmed = await ConfirmAlert(
      `Are you sure you want to remove the user ${user.user_first_name} ${user.user_last_name}? This action cannot be undone.`,
    );
    if (confirmed) {
      try {
  ...
  const handleArchiveUser = async (user) => {
    const confirmed = await ConfirmAlert(
      `Are you sure you want to archive ${user.user_first_name} ${user.user_last_name}?`,
    );
    if (confirmed) {
      try {
  ...
  const handleRestoreUser = async (user) => {
    const confirmed = await ConfirmAlert(
      `Are you sure you want to restore ${user.user_first_name} ${user.user_last_name}?`,
    );
    if (confirmed) {
      try {
  ...
  const handleHardDeleteUser = async (user) => {
    const confirmed = await ConfirmAlert(
      `Are you sure you want to PERMANENTLY delete ${user.user_first_name} ${user.user_last_name}? This cannot be undone.`,
    );
    if (confirmed) {
      try {
        ...
        <div className="domain-overview-header d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
          <div>
            ...
            <p className="domain-overview-subtitle text-muted mb-0">
              You have {totalUsers} {isArchivedView ? "archived " : ""}user(s)
              on your account
            </p>
          </div>
                    ...
                    users.map((user) => {
                      ...
                      return (
                        <tr key={user._id}>
                          ...
                          <td>
                            <span
                              className={`badge ${user.user_status === "active" ? "bg-soft-success text-success" : "bg-soft-secondary text-secondary"}`}
                            >
                              {user.user_status}
                            </span>
                          </td>
