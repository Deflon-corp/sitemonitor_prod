    ...
    <table className="table table-hover table-striped table-borderless align-middle mb-0">
      <thead>
        <tr className="border-bottom border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50">
          ...
          <th
            className="fw-semibold text-body py-2 text-end"
            style={{ width: 100 }}
          >
            Occurrences
          </th>
              ...
              <div className="d-flex align-items-start gap-2">
                ...
                <div>
                  ...
                  <div className="d-flex flex-wrap align-items-center gap-2 mt-1 text-muted fs-12">
                    ...
                    <i
                      className="isax isax-document-text fs-14 ms-1"
                      aria-hidden="true"
                    />
...
const AccessibilitySection = ({ data, score, domainId, pageUrl }) => {
  ...
  const dynamicIssues = useMemo(() => {
    if (!effectiveData?.issues || !Array.isArray(effectiveData.issues))
      return [];

    const passed = effectiveData.passedCount || 0;
    const failed =
      effectiveData.failedCount || effectiveData.issues.length || 0;
    const totalChecks = passed + failed;
    const computedEffect =
      totalChecks > 0 ? (100 / totalChecks).toFixed(2) + " %" : "0.00 %";
    ...
    return effectiveData.issues.map((issue) => ({
      ...
      successCriteria:
        issue.description || issue.successCriteria || "WCAG Check",
      description:
        issue.description || issue.help || "No description available.",
  ...
  const levelA = useMemo(
    () => dynamicIssues.filter((i) => i.level === "A"),
    [dynamicIssues],
  );
  const levelAA = useMemo(
    () => dynamicIssues.filter((i) => i.level === "AA"),
    [dynamicIssues],
  );
  ...
  useEffect(() => {
    if (!selectedCheckId) {
      if (levelFilter === "all" || levelFilter === "a")
        setSelectedCheckId(levelA[0]?.id || null);
      else if (levelFilter === "aa") setSelectedCheckId(levelAA[0]?.id || null);
    }
  }, [levelFilter, levelA, levelAA, selectedCheckId]);

  const overallPercent =
    score !== undefined ? score : effectiveData?.score || 100;
  const levelAPercent =
    effectiveData?.levelAScore !== undefined
      ? effectiveData.levelAScore
      : overallPercent;
  const levelAAPercent =
    effectiveData?.levelAAScore !== undefined
      ? effectiveData.levelAAScore
      : overallPercent;
  ...
  const exportAccessibilityPDF = useCallback(async () => {
    ...
    const head = [
      ["Check", "Level", "Roles", "Success criteria", "Individual issues"],
    ];
          ...
          <div className="d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div className="d-flex align-items-center gap-2">
              ...
              <div>
                ...
                <p className="text-muted fs-13 mb-0">
                  How accessible this page is for everyone.
                </p>
              </div>
            </div>
            <div className="d-flex align-items-center gap-3">
              ...
              <div className="d-flex align-items-center gap-2">
                <ComplianceRing
                  percent={
                    typeof overallPercent === "number" && !isNaN(overallPercent)
                ...
                <div>
                  ...
                  >
                    Overall score for how usable this page is.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="card border-0 shadow-sm mb-3">
        <div className="card-body py-2">
          <div className="d-flex flex-wrap align-items-center gap-3">
            <nav
              className="nav nav-tabs border-0 gap-2"
              aria-label="Priority level filter"
            >
      ...
      {levelFilter === "a" && (
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-body">
            <LevelHeaderBlock
              badgeLabel="A"
              title="High Priority (Level A) issues."
              percent={levelAPercent}
            />
          </div>
        </div>
      )}
      ...
      {levelFilter === "aa" && (
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-body">
            <LevelHeaderBlock
              badgeLabel="AA"
              title="Medium Priority (Level AA) issues."
              percent={levelAAPercent}
            />
          </div>
        </div>
      )}
                ...
                <>
                  <div className="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <span className="fw-semibold text-primary">
                      {selectedCheck.individualIssues}
                    </span>
                  </div>
                  <div className="d-flex align-items-center gap-2 mb-2">
                    ...
                    >
                      {selectedCheck.level === "AA" ? (
                        ...
                        <i
                          className="isax isax-danger fs-16"
                          aria-hidden="true"
                        />
                      )}
                    </span>
                  ...
                  <div className="mb-3">
                    <i
                      className="isax isax-teacher text-primary me-1"
                      aria-hidden="true"
                    />
                    <span className="fs-13 text-muted">
                      Learn why this matters and how to fix it
                    </span>
                  </div>
                    ...
                    {(selectedCheck?.nodes || []).map((inst, idx) => (
                      <div key={inst.id || idx} className="mb-4">
                        <div className="d-flex align-items-center gap-2 mb-2">
                          ...
                          >
                            <i
                              className="isax isax-shield-tick text-primary fs-18"
                              aria-hidden="true"
                            />
                          </span>
                              ...
                              onClick={() => {
                                if (selectedCheck) {
                                  setIssueDrawerData({
                                    ...
                                    element:
                                      (inst.target && inst.target.join(", ")) ||
                                      "Element",
                                    dateFound: effectiveData?.scanDate
                                      ? new Date(
                                          effectiveData.scanDate,
                                        ).toLocaleDateString()
                                      : new Date().toLocaleDateString(),
                                    effectOnCompliance:
                                      selectedCheck.effectOnCompliance ||
                                      "0.00 %",
                                    impact: selectedCheck.impact || "moderate",
                                    snippetHtml:
                                      inst.snippet || "No snippet available",
                                    pageTitle: effectiveData?.title || "Page",
                                    pageUrl:
                                      effectiveData?.url || pageUrl || "",
                                    responsibility: selectedCheck.roles,
                                    successCriteria:
                                      selectedCheck.successCriteria
                                        ?.replace(
                                          /^Part of success criteria\s*/i,
                                          "",
                                        )
                                        .trim() || "1.1.1",
                                    difficulty: "Medium",
                        ...
                        <div className="rounded bg-danger bg-opacity-10 border border-danger border-opacity-25 overflow-hidden">
                          {inst.target && inst.target.length > 0 && (
                            <div
                              className="px-3 pt-3 pb-1 fs-13 text-danger fw-semibold"
                          ...
                          <pre
                            ...
                            style={{
                              fontSize: "0.75rem",
                              maxHeight: expandedSnippets.has(inst.id || idx)
                                ? "none"
                                : 80,
                            }}
                          >
                          ...
                          {(inst.snippet?.length > 150 ||
                            ...
                            <div className="px-3 py-2 border-top border-danger border-opacity-25 bg-danger bg-opacity-10">
                              {!expandedSnippets.has(inst.id || idx) ? (
                                <button
                                  type="button"
                                  className="btn btn-link p-0 text-primary small text-decoration-none fw-medium fs-13"
                                  onClick={() => toggleSnippet(inst.id || idx)}
                                >
                                  Show more
                                </button>
                              ) : (
