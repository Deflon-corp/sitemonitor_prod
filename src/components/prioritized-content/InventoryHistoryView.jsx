...
const CATEGORY_CONFIG = {
  "html-pages": { key: "htmlPages", color: CRAWLED_COLOR, fill: "rgba(59, 130, 246, 0.12)", label: "HTML Pages" },
  "images": { key: "images", color: IMAGES_COLOR, fill: "rgba(34, 197, 94, 0.1)", label: "Images" },
  "css": { key: "css", color: CSS_COLOR, fill: "rgba(245, 158, 11, 0.1)", label: "CSS" },
  "js": { key: "js", color: JS_COLOR, fill: "rgba(234, 179, 8, 0.1)", label: "JavaScript" },
  "documents": { key: "documents", color: DOCUMENTS_COLOR, fill: "rgba(139, 92, 246, 0.1)", label: "Documents" },
  "emails": { key: "emails", color: EMAILS_COLOR, fill: "rgba(239, 68, 68, 0.1)", label: "Emails" },
  "headlinks": { key: "headlinks", color: HEADLINKS_COLOR, fill: "rgba(6, 182, 212, 0.1)", label: "Headlinks" },
  "links": { key: "links", color: CRAWLED_COLOR, fill: "rgba(59, 130, 246, 0.1)", label: "Links" },
  "forms": { key: "forms", color: IMAGES_COLOR, fill: "rgba(34, 197, 94, 0.1)", label: "Forms" },
  "iframes": { key: "iframes", color: CSS_COLOR, fill: "rgba(245, 158, 11, 0.1)", label: "IFrames" },
  "frames": { key: "frames", color: JS_COLOR, fill: "rgba(234, 179, 8, 0.1)", label: "Frames" },
};
...
export default function InventoryHistoryView({ domainId, category }) {
  ...
  useEffect(() => {
    if (!domainId) return;
    setLoading(true);
    inventoryApi.getInventoryHistory(domainId)
      .then(res => {
  ...
  const dynamicCrawledData = hasHistory ? history.map(h => h.htmlPages || 0) : Array(X_LABELS.length).fill(0);
  const dynamicImagesData = hasHistory ? history.map(h => h.images || 0) : Array(X_LABELS.length).fill(0);
  const dynamicDocsData = hasHistory ? history.map(h => h.documents || 0) : Array(X_LABELS.length).fill(0);
  const dynamicCssData = hasHistory ? history.map(h => h.css || 0) : Array(X_LABELS.length).fill(0);
  const dynamicJsData = hasHistory ? history.map(h => h.js || 0) : Array(X_LABELS.length).fill(0);
  const dynamicEmailsData = hasHistory ? history.map(h => h.emails || 0) : Array(X_LABELS.length).fill(0);
  const dynamicHeadlinksData = hasHistory ? history.map(h => h.headlinks || 0) : Array(X_LABELS.length).fill(0);
  ...
  return (
    <div className="card border-0 shadow-sm mb-4 bg-white rounded-4 overflow-hidden">
      <div className="card-body p-4">
        ...
        <div className="position-relative" style={{ maxWidth: CHART_WIDTH, margin: "0 auto" }}>
          <svg width="100%" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} style={{ overflow: "visible" }} aria-hidden="true">
            {dynamicYTicks.map((v) => {
              const y = yScale(v);
            ...
            {displayLabels.map((label, i) => (
              <text key={`${label}-${i}`} x={xScale(i)} y={CHART_HEIGHT - 6} textAnchor="middle" fill="#6b7280" style={{ fontSize: 10 }}>{label}</text>
            ))}
            
            <line x1={PADDING.left} y1={baselineY} x2={PADDING.left + PLOT_WIDTH} y2={baselineY} stroke="rgba(0,0,0,0.08)" strokeWidth="1" />
            
            <path d={crawledAreaPath} fill={CATEGORY_CONFIG["html-pages"].fill} />
