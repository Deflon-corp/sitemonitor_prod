...
const DictionarySection = ({
  ...
  const exportCSV = useCallback(() => {
    ...
    const body = items
      .map(
        (r) =>
          `"${r.word.replace(/"/g, '""')}","${r.language.replace(/"/g, '""')}","${r.dateAdded}",${r.pages}`,
      )
      .join("\n");
    downloadBlob(
  ...
  const exportPDF = useCallback(async () => {
    ...
    autoTable(doc, {
      head: [["Word", "Language", "Date added", "Pages"]],
      body: items.map((r) => [
        r.word,
        r.language,
        r.dateAdded,
            ...
            <thead>
              <tr className="border-bottom border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50">
                ...
                {variant === "page" && (
                  <>
                    <th
                      className="py-3 pe-4 text-body fs-13 fw-semibold text-nowrap text-center"
                  ...
                  {variant === "page" && (
                    <td className="py-3 pe-2 text-center align-middle">
                      <button
                        type="button"
                        className="btn btn-icon btn-sm btn-light border-0 rounded-2 text-primary"
                        title="Open dictionary entry page"
                        onClick={() => onOpenIssue?.(row.id)}
                        aria-label="Open dictionary entry page"
                      >
                        <i
                          className="isax isax-info-circle fs-18"
                          aria-hidden="true"
