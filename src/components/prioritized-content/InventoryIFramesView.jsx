...
import PagesWithIFrameDrawer from "./PagesWithIFrameDrawer";
          ...
          <div className="card-body">
            <div className="d-flex flex-wrap align-items-center justify-content-between gap-3">
              ...
              <div className="flex-grow-1 flex-md-grow-0" style={{ minWidth: 200, maxWidth: 320 }}>
                <input type="search" className="form-control form-control-sm" placeholder="Search..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} aria-label={`Search ${TITLE.toLowerCase()}`} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="card-body p-0">
              ...
              <table className="table table-hover table-striped table-borderless align-middle mb-0">
                ...
                <tbody>
                  {detailsPaginated.map((row) => (
                    <tr key={row.id}>
                      <td className="px-4 py-2"><a href={row.link} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-none text-break">{row.link}</a></td>
            ...
            <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 px-4 py-3 border-top">
              <div className="d-flex align-items-center gap-2">
                ...
                <select className="form-select form-select-sm" style={{ width: "auto" }} value={rowsPerPage} onChange={(e) => { setRowsPerPage(Number(e.target.value)); setCurrentPage(1); }}>{ROWS_PER_PAGE_OPTIONS.map((n) => <option key={n} value={n}>{n}</option>)}</select>
                <span className="text-muted small">{(currentPage - 1) * rowsPerPage + 1}-{Math.min(currentPage * rowsPerPage, detailsFiltered.length)} of {detailsFiltered.length}</span>
              </div>
              <nav aria-label={`${TITLE} pagination`}><ul className="pagination pagination-sm mb-0">
                <li className={`page-item ${currentPage <= 1 ? "disabled" : ""}`}><button type="button" className="page-link" onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage <= 1} aria-label="Previous">Previous</button></li>
                {Array.from({ length: detailsTotalPages }, (_, i) => i + 1).map((p) => <li key={p} className={`page-item ${currentPage === p ? "active" : ""}`}><button type="button" className="page-link" onClick={() => setCurrentPage(p)}>{p}</button></li>)}
                <li className={`page-item ${currentPage >= detailsTotalPages ? "disabled" : ""}`}><button type="button" className="page-link" onClick={() => setCurrentPage((p) => Math.min(detailsTotalPages, p + 1))} disabled={currentPage >= detailsTotalPages} aria-label="Next">Next</button></li>
              </ul></nav>
            </div>
          </div>
        </div>
    ...
    <>
      <div className="mb-4">
        <div className="d-flex align-items-center gap-2 mb-1">
          <span className="avatar avatar-40 rounded bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center" style={{ minWidth: 40, height: 40 }}><i className={`isax ${ICON} fs-4`} aria-hidden /></span>
      ...
      <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
        ...
        <div className="d-flex align-items-center gap-2 forms-download-report-btn">
          ...
          <div className="position-relative" style={{ minWidth: 200, maxWidth: 280 }}>
            ...
            <span className="position-absolute end-0 top-50 translate-middle-y me-2 text-muted" style={{ pointerEvents: "none" }}><i className="isax isax-search-normal small" aria-hidden /></span>
          </div>
        </div>
      </div>
      <div className="card border border-secondary border-opacity-25 rounded-3 shadow-sm overflow-hidden">
        <div className="card-body p-0">
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="table-light"><tr><th className="border-0 ps-4 py-3 fw-semibold text-body text-uppercase small">URL</th><th className="border-0 pe-4 py-3 text-end fw-semibold text-body text-uppercase small" style={{ width: 80 }}>PAGE</th></tr></thead>
              <tbody>
                {paginatedItems.map((row) => (
                  <tr key={row.id}>
                    <td className="ps-4 py-3"><a href={row.url} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-none text-break">{row.url}</a></td>
                    <td className="pe-4 py-3 text-end">
                      <button type="button" className="btn btn-link p-0 border-0 d-flex flex-column align-items-end text-decoration-none" onClick={() => { setSelectedResourceUrl(row.url); setPagesDrawerOpen(true); }} aria-label={`Pages with this iframe (${row.pageCount} page${row.pageCount !== 1 ? "s" : ""})`}>
                        <span className="fw-semibold text-primary" style={{ fontSize: "1.125rem" }}>{row.pageCount}</span><span className="text-muted text-uppercase small" style={{ fontSize: "0.7rem" }}>PAGE</span>
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      <PagesWithIFrameDrawer open={pagesDrawerOpen} onClose={() => { setPagesDrawerOpen(false); setSelectedResourceUrl(null); }} resourceUrl={selectedResourceUrl} onOpenPageDetails={(row) => { setSelectedPageForDetails(row); setPagesDrawerOpen(false); setPageDetailsOpen(true); }} />
