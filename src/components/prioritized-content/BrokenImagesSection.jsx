...
export const BROKEN_IMAGES_SAMPLE = [
  ...
  {
    id: 3,
    url: "https://example.com/old/thumbnail.gif",
    responseCode: "404",
    type: "image",
  ...
  {
    id: 4,
    url: "https://cdn.example.com/retired/product.webp",
    responseCode: "410",
    type: "image",
  ...
  {
    id: 5,
    url: "https://example.com/images/partner-icon.svg",
    responseCode: "404",
    type: "image",
...
const BrokenImagesSection = ({ items = BROKEN_IMAGES_SAMPLE, onOpenIssue }) => {
  ...
  const exportCSV = useCallback(() => {
    ...
    const body = items
      .map(
        (r) =>
          `"${r.url.replace(/"/g, '""')}","${r.responseCode}","${r.type}","${r.dateFound}"`,
      )
      .join("\n");
          ...
          <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 px-4 py-3 border-top">
            <div className="d-flex align-items-center gap-2">
              ...
              <span className="text-muted small">
                ...
                {Math.min(currentPage * rowsPerPage, items.length)} of{" "}
                {items.length}
              </span>
            </div>
