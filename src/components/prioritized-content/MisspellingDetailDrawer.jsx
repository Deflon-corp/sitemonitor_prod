...
export default function MisspellingDetailDrawer({
  ...
  const resolvedPagesWithMisspelling = useMemo(() => {
    if (pagesWithMisspelling && pagesWithMisspelling.length > 0) return pagesWithMisspelling;
    if (page) return [{ title: page.title || "Untitled Page", url: page.url, language: "N/A", misspellings: 0, potentialMisspellings: 0, views: 0 }];
    return [];
  ...
  const filteredPages = useMemo(() => {
    ...
    return resolvedPagesWithMisspelling.filter((p) => p.title.toLowerCase().includes(q) || p.url.toLowerCase().includes(q));
  ...
  const handleCopyUrl = () => {
    const url = issue && resolvedPagesWithMisspelling.length > 0 ? resolvedPagesWithMisspelling[0].url : window.location.href;
              ...
              , React.createElement('div', { className: "d-flex flex-wrap align-items-center justify-content-between gap-2 px-4 py-3 border-top" }
                , React.createElement('div', { className: "d-flex align-items-center gap-2" }
                  ...
                  , React.createElement('span', { className: "text-muted small" }
                    , resolvedPagesWithMisspelling.length === 0 ? 0 : (currentPage - 1) * rowsPerPage + 1, "–", Math.min(currentPage * rowsPerPage, resolvedPagesWithMisspelling.length), " of ", resolvedPagesWithMisspelling.length
                  )
                )
