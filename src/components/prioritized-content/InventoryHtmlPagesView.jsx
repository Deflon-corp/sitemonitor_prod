...
export default function InventoryHtmlPagesView({ items = [], variant }) {
  ...
  const exportCSV = useCallback(() => {
    ...
    const body = sortedRows
      .map(
        (r) =>
          `"${(r.title || "").replace(/"/g, '""')}","${r.url.replace(/"/g, '""')}",${r.notifications},${r.views}`,
      )
      .join("\n");
    downloadBlob(
                      ...
                      <td className="px-4 py-2">
                        <div className="d-flex flex-column">
                          ...
                          <a
                            ...
                            className="text-primary text-decoration-none text-break small mt-1"
                          >
                            {row.url}
                          </a>
                        </div>
        ...
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-body">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                <h6 className="mb-0 fw-semibold text-body">HTML Pages</h6>
                <p className="text-muted fs-13 mb-0">
                  {sortedRows.length}
                  {" content"}
                </p>
        ...
        <div className="card-body py-3">
          <div className="d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div className="d-flex align-items-center gap-2">
              ...
              >
                <i className="isax isax-document-copy" aria-hidden={true} />
                All
              </button>
            </div>
            <div className="d-flex align-items-center gap-2">
                      ...
                      <div className="dropdown">
                        ...
                        >
                          <span className="badge bg-primary bg-opacity-10 text-primary rounded-pill">
                            {String(row.notifications || 0)}
                          </span>
                        </button>
                        <QAQuickInfoMenu
                          brokenLinks={
                            Array.isArray(row.brokenLinks)
                              ? row.brokenLinks.length
                              : Number(row.brokenLinks) || 0
                          }
                          brokenImages={
                            Array.isArray(row.brokenImages)
                              ? row.brokenImages.length
                              : Number(row.brokenImages) || 0
                          }
                          misspellings={
                            Array.isArray(row.misspellings)
                              ? row.misspellings.length
                              : Number(row.misspellings) || 0
                          }
                          policies={
                          ...
                          accessibility={
                            row.accessibility?.violations?.length ||
                            Number(row.accessibility?.score) ||
                            0
                          }
                          seo={
                            Array.isArray(row.seo)
                              ? row.seo.length
                              : Number(row.seo) || 0
                          }
