...
export default function DocumentDetailsDrawer({ open, onClose, document: documentRow }) {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(DEFAULT_ROWS_PER_PAGE);
  const [sortViewsAsc, setSortViewsAsc] = useState(true);

  const documentLink = _nullishCoalesce(_optionalChain([documentRow, 'optionalAccess', _2 => _2.link]), () => ( ""));
  ...
  const reportBase = safeFilename("Pages-with-Document-Report");
  const exportCSV = useCallback(() => {
    const header = "Title,URL,Type,Response code,Views\n";
    const body = sortedRows
      .map(
        (r) =>
          `"${(r.title || "").replace(/"/g, '""')}","${r.url.replace(/"/g, '""')}","${r.type}",${r.responseCode},${r.views}`
      )
      .join("\n");
  ...
  const exportPDF = useCallback(async () => {
    ...
    autoTable(doc, {
      head: [["Title", "URL", "Type", "Response code", "Views"]],
      body: sortedRows.map((r) => [r.title || "", r.url, r.type, String(r.responseCode), String(r.views)]),
      startY: 10,
      styles: { fontSize: 8 },
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: "wrap" }, 2: { cellWidth: 22 }, 3: { cellWidth: 22 }, 4: { cellWidth: 18 } },
    });
  ...
  const content = (
    React.createElement(React.Fragment, null
      ...
      , React.createElement('div', {
        ...
        style: { zIndex: 1055, width: "min(100%, 1400px)", overflow: "visible" },
        role: "dialog",
        'aria-modal': "true",
        'aria-labelledby': "document-details-drawer-title"}
          ...
          , React.createElement('div', { className: "d-flex align-items-start justify-content-between gap-3"   }
            , React.createElement('div', { className: "min-w-0 flex-grow-1" }
              , React.createElement('div', { className: "d-flex align-items-center gap-2 mb-1"   }
                ...
                , React.createElement('h5', { id: "document-details-drawer-title", className: "mb-0 fw-semibold text-body"  }, "Document details"

                )
              )
              , React.createElement('p', { className: "text-muted small mb-0 ms-4 text-break"    }, documentLink || "—")
              , documentRow && (
                React.createElement('p', { className: "text-muted small mb-0 ms-4 mt-1"    }, "Notifications: "
                   , documentRow.notifications, " · Views: "   , documentRow.views
                )
              )
            )
