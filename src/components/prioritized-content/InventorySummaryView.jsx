...
const InventorySummaryView = ({ data }) => {
  ...
  if (history.length === 1) {
    ...
    history = [{ ...pt, date: prevDate.toISOString() }, pt];
  }
  ...
  const dynamicCrawledData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.htmlPages
          ? data.htmlPages
          : h.htmlPages || 0,
      )
    : CRAWLED_PAGES_DATA;
  const dynamicImagesData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.images ? data.images : h.images || 0,
      )
    : IMAGES_DATA;
  const dynamicDocsData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.documents
          ? data.documents
          : h.documents || 0,
      )
    : DOCS_DATA;
  const dynamicCssData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.css ? data.css : h.css || 0,
      )
    : CSS_DATA;
  const dynamicJsData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.js ? data.js : h.js || 0,
      )
    : JS_DATA;
  const dynamicEmailsData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.emails ? data.emails : h.emails || 0,
      )
    : EMAILS_DATA;
  const dynamicHeadlinksData = hasHistory
    ? history.map((h, i) =>
        i === history.length - 1 && data?.headlinks
          ? data.headlinks
          : h.headlinks || 0,
      )
    : HEADLINKS_DATA;
  ...
  const technicalItems = [
    ...
    },
    {
      ...
      active: (data?.frames || 0) > 0,
    },
    {
      label: "IFrames",
      value: data?.iframes || 0,
      icon: "isax-code-circle",
      active: (data?.iframes || 0) > 0,
    },
  ];
  ...
  const linksItems = [
    {
      ...
      value: data?.links || 0,
      icon: "isax-link-2",
      active: (data?.links || 0) > 0,
    },
    {
      label: "Emails",
      value: data?.emails || 0,
      icon: "isax-sms",
      active: (data?.emails || 0) > 0,
    },
    {
      label: "Headlinks",
      value: data?.headlinks || 0,
      icon: "isax-link-2",
      active: (data?.headlinks || 0) > 0,
    },
  ];
  ...
  const contentItems = [
    {
      label: "HTML Pages",
      value: data?.htmlPages || 0,
      icon: "isax-folder",
      active: (data?.htmlPages || 0) > 0,
    },
    {
      label: "Documents",
      value: data?.documents || 0,
      icon: "isax-document-text",
      active: (data?.documents || 0) > 0,
    },
    {
      label: "Images",
      value: data?.images || 0,
      icon: "isax-image",
      active: (data?.images || 0) > 0,
    },
  ];
  ...
  return (
    <div className="inventory-summary-view d-flex flex-column gap-4">
      ...
      {/* Inventory History */}
      <div
        className="card border border-secondary border-opacity-25 rounded-3 overflow-hidden bg-transparent"
        ...
        <div className="card-body p-4 bg-transparent">
          <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            ...
            <div className="d-flex flex-wrap align-items-center gap-4">
              ...
              >
                <i
                  className="isax isax-calendar-1 text-primary"
                  style={{ fontSize: "0.9rem" }}
                  aria-hidden="true"
                />
                {selectedYear}
                <i
                  className="isax isax-arrow-down-1 small"
                  aria-hidden="true"
                />
              </button>
            </div>
            ...
            {hoveredIndex != null && (
              ...
              >
                <div className="mb-2 fw-semibold border-bottom border-secondary pb-1">
                  {displayLabels[hoveredIndex]?.date}{" "}
                  {displayLabels[hoveredIndex]?.time}
                </div>
          ...
          <div className="row g-3 mt-3">
            <div className="col-12 col-md-6">
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle at 100% 0%, rgba(147, 112, 219, 0.14) 0%, transparent 52%)",
                  backgroundColor: "#fff",
              ...
              >
                <p className="text-muted small mb-1">
                  Total Crawled pages this year
                </p>
                <div className="d-flex flex-wrap align-items-baseline gap-2">
                  <span className="fw-bold fs-4 text-body">
                    {data?.htmlPages || 0}
                  </span>
                  <span className="small text-success d-inline-flex align-items-center gap-1 fw-medium">
                    <span
                      className="d-inline-flex align-items-center justify-content-center rounded-circle bg-success bg-opacity-15"
                      style={{ width: 20, height: 20 }}
                    >
                      <i
                        className="isax isax-arrow-up-1"
            ...
            <div className="col-12 col-md-6">
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle at 100% 0%, rgba(34, 197, 94, 0.14) 0%, transparent 52%)",
                  backgroundColor: "#fff",
                  backgroundRepeat: "no-repeat",
                  boxShadow: "0 1px 3px rgba(0,0,0,0.06)",
                }}
              ...
              <div className="row g-0">
                {technicalItems.map((item, idx) => (
                  <div key={item.label} className="col-6">
                    <div
                      className={`d-flex align-items-center gap-3 p-3 ${idx % 2 === 0 ? "border-end border-secondary border-opacity-25" : ""} ${idx < 2 ? "border-bottom border-secondary border-opacity-25" : ""}`}
                    >
                      <span
                        className={`avatar avatar-40 avatar-rounded d-flex align-items-center justify-content-center flex-shrink-0 ${item.active ? "bg-primary bg-opacity-10 text-primary" : "bg-secondary bg-opacity-10 text-secondary"}`}
                      >
                        <i
                      ...
                      <div className="min-w-0">
                        ...
                        <p
                          className="fw-semibold mb-0 text-body"
                          style={{ fontSize: "1rem" }}
                        >
              ...
              <div className="row g-0">
                {linksItems.map((item, idx) => (
                  <div key={item.label} className="col-6 col-md-4">
                    <div
                      className={`d-flex align-items-center gap-3 p-3 border-end border-secondary border-opacity-25 ${idx < 3 ? "border-bottom border-secondary border-opacity-25" : ""}`}
                    >
                      <span
                        className={`avatar avatar-40 avatar-rounded d-flex align-items-center justify-content-center flex-shrink-0 ${item.active ? "bg-primary bg-opacity-10 text-primary" : "bg-secondary bg-opacity-10 text-secondary"}`}
                      >
                ...
                <div className="col-6 col-md-4">
                  <div
                    className="p-3 border-end border-secondary border-opacity-25"
                    style={{ minHeight: 76 }}
                    aria-hidden="true"
                  />
                </div>
            ...
            <div className="card-body p-4">
              ...
              <div className="d-flex flex-column gap-0">
                {contentItems.map((item, idx) => (
                  <div
                    key={item.label}
                    className={`d-flex align-items-center gap-3 p-3 ${idx < contentItems.length - 1 ? "border-bottom border-secondary border-opacity-25" : ""}`}
                  >
                    <span
                      className={`avatar avatar-40 avatar-rounded d-flex align-items-center justify-content-center flex-shrink-0 ${item.active ? "bg-primary bg-opacity-10 text-primary" : "bg-secondary bg-opacity-10 text-secondary"}`}
                    >
