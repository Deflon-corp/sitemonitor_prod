...
const BrokenLinkIssueDrawer = ({
  ...
  const resolvedPagesWithLink = useMemo(() => {
    if (pagesWithLink && pagesWithLink.length > 0) return pagesWithLink;
    if (page) return [{ title: page.title || "Untitled Page", url: page.url, priority: "Medium", views: 0 }];
    return [];
  }, [pagesWithLink, page]);

  const totalPages = Math.max(1, Math.ceil(resolvedPagesWithLink.length / rowsPerPage));
  const paginatedPages = useMemo(() => {
    const start = (currentPage - 1) * rowsPerPage;
    return resolvedPagesWithLink.slice(start, start + rowsPerPage);
  }, [resolvedPagesWithLink, currentPage, rowsPerPage]);
            ...
            <div className="card-body p-0">
              ...
              <div className="table-responsive">
                <table className="table table-hover table-striped table-borderless align-middle mb-0">
                  ...
                  <tbody>
                    {resolvedPagesWithLink.length === 0 && (
                      <tr>
              ...
              <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 px-4 py-3 border-top">
                <div className="d-flex align-items-center gap-2">
                  ...
                  <span className="text-muted small">
                    {resolvedPagesWithLink.length === 0 ? 0 : (currentPage - 1) * rowsPerPage + 1}-{Math.min(currentPage * rowsPerPage, resolvedPagesWithLink.length)} of {resolvedPagesWithLink.length}
                  </span>
                </div>
