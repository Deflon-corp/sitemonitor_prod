...
import PagesWithFrameDrawer from "./PagesWithFrameDrawer";
...
const REPORT_BASE = "Frames-Report";
const DEFAULT_INVENTORY_SUB_VIEW = "frames";

export default function InventoryFramesView({ items = SAMPLE, variant }) {
          ...
          <div className="card-body p-0">
            <div className="table-responsive">
              <table className="table table-hover table-striped table-borderless align-middle mb-0">
                ...
                <tbody>
                  {detailsPaginated.map((row) => (<tr key={row.id}><td className="px-4 py-2"><a href={row.link} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-none text-break">{row.link}</a></td><td className="px-4 py-2"><span className="badge bg-secondary bg-opacity-25 text-body">{row.type}</span></td><td className="px-4 py-2 text-body">{row.responseCode}</td></tr>))}
                </tbody>
              </table>
            </div>
  ...
  return (
    <>
      ...
      <div className="card border border-secondary border-opacity-25 rounded-3 shadow-sm overflow-hidden">
        <div className="card-body p-0">
          <div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead className="table-light"><tr><th className="border-0 ps-4 py-3 fw-semibold text-body text-uppercase small">URL</th><th className="border-0 pe-4 py-3 text-end fw-semibold text-body text-uppercase small" style={{ width: 80 }}>PAGE</th></tr></thead><tbody>{paginatedItems.map((row) => (<tr key={row.id}><td className="ps-4 py-3"><a href={row.url} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-none text-break">{row.url}</a></td><td className="pe-4 py-3 text-end"><button type="button" className="btn btn-link p-0 border-0 d-flex flex-column align-items-end text-decoration-none" onClick={() => { setSelectedResourceUrl(row.url); setPagesDrawerOpen(true); }} aria-label={`Pages with this frame (${row.pageCount} page${row.pageCount !== 1 ? "s" : ""})`}><span className="fw-semibold text-primary" style={{ fontSize: "1.125rem" }}>{row.pageCount}</span><span className="text-muted text-uppercase small" style={{ fontSize: "0.7rem" }}>PAGE</span></button></td></tr>))}</tbody></table></div>
      ...
      <PagesWithFrameDrawer open={pagesDrawerOpen} onClose={() => { setPagesDrawerOpen(false); setSelectedResourceUrl(null); }} resourceUrl={selectedResourceUrl} onOpenPageDetails={(row) => { setSelectedPageForDetails(row); setPagesDrawerOpen(false); setPageDetailsOpen(true); }} />
