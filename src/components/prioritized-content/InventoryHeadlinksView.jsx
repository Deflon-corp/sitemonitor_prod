...
import PagesWithHeadlinkDrawer from "./PagesWithHeadlinkDrawer";
...
export default function InventoryHeadlinksView({ items = [], variant }) {
  ...
  const exportPDF = useCallback(async () => {
    ...
    autoTable(doc, {
      head: [["URL", "PAGE"]],
      body: filteredItems.map((r) => [r.url || "", String(r.pageCount ?? "")]),
      startY: 10,
      styles: { fontSize: 8 },
    });
            ...
            <div className="d-flex flex-wrap align-items-center justify-content-between gap-3">
              <div className="d-flex align-items-center gap-2">
                <span className="avatar avatar-40 avatar-rounded bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center">
                  <i className={`isax ${ICON} fs-22`} aria-hidden />
                </span>
                <div>
              ...
              <nav aria-label={`${TITLE} pagination`}>
                <ul className="pagination pagination-sm mb-0">
                  ...
                  <li className={`page-item ${currentPage >= detailsTotalPages ? "disabled" : ""}`}>
                    <button type="button" className="page-link" onClick={() => setCurrentPage((p) => Math.min(detailsTotalPages, p + 1))} disabled={currentPage >= detailsTotalPages} aria-label="Next">Next</button>
                  </li>
                </ul>
              </nav>
            </div>
    ...
    <>
      <div className="mb-4">
        <div className="d-flex align-items-center gap-2 mb-1">
          <span className="avatar avatar-40 rounded bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center" style={{ minWidth: 40, height: 40 }}>
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="table-light">
                <tr>
                  <th className="border-0 ps-4 py-3 fw-semibold text-body text-uppercase small">URL</th>
                  <th className="border-0 pe-4 py-3 text-end fw-semibold text-body text-uppercase small" style={{ width: 80 }}>PAGE</th>
                </tr>
              </thead>
              <tbody>
                {paginatedItems.map((row) => (
                  <tr key={row.id}>
                    <td className="ps-4 py-3">
                      <a href={row.url} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-none text-break">{row.url}</a>
                    </td>
                    <td className="pe-4 py-3 text-end">
                      <button type="button" className="btn btn-link p-0 border-0 d-flex flex-column align-items-end text-decoration-none" onClick={() => { setSelectedResourceUrl(row.url); setPagesDrawerOpen(true); }} aria-label={`Pages with this headlink (${row.pageCount} page${row.pageCount !== 1 ? "s" : ""})`}>
                        <span className="fw-semibold text-primary" style={{ fontSize: "1.125rem" }}>{row.pageCount}</span>
                        <span className="text-muted text-uppercase small" style={{ fontSize: "0.7rem" }}>PAGE</span>
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      <PagesWithHeadlinkDrawer open={pagesDrawerOpen} onClose={() => { setPagesDrawerOpen(false); setSelectedResourceUrl(null); }} resourceUrl={selectedResourceUrl} onOpenPageDetails={(row) => { setSelectedPageForDetails(row); setPagesDrawerOpen(false); setPageDetailsOpen(true); }} />
