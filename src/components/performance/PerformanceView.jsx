...
const PerformanceView = () => {
  ...
  const handleRunScan = async () => {
    ...
    try {
      ...
      if (response.success) {
        setScanMessage(
          "Scan triggered successfully! Page speed metrics and Core Web Vitals are compiling in the background...",
        );
        ...
        setScanMessage(
          response.message || "Failed to trigger scan. Please try again.",
        );
        setScanning(false);
      }
    } catch (err) {
  ...
  return (
    <div className="performance-view">
      ...
      <div className="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-4">
        <div>
          ...
          <p className="text-muted fs-13 mb-0">
            ...
            optimize your Core Web Vitals to elevate your user experience.
          </p>
        </div>
        {domainId &&
          (scanning ? (
            <div
              className="d-flex align-items-center gap-2 py-2 px-3 text-primary bg-primary bg-opacity-10 rounded-2 fw-medium border border-primary border-opacity-25 shadow-sm animate-pulse"
              style={{ minHeight: 38 }}
            >
              <span
      ...
      {scanMessage && (
        <div
          className={`alert border-0 shadow-sm mb-4 d-flex align-items-center justify-content-between ${scanMessage.includes("successfully") ? "alert-success text-success bg-success bg-opacity-10" : "alert-danger text-danger bg-danger bg-opacity-10"}`}
          role="alert"
        >
          <div className="d-flex align-items-center gap-2">
            <i
              className={`isax ${scanMessage.includes("successfully") ? "isax-tick-circle5" : "isax-info-circle5"} fs-18`}
            />
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <p className="mb-0 text-muted fs-13">
                Please select a domain in the sidebar to review page performance
                metrics.
              </p>
            </div>
          </div>
                ...
                <div className="card-body d-flex align-items-center gap-4 py-4">
                  ...
                  >
                    ...
                    <div className="position-absolute top-50 start-50 translate-middle text-center">
                      <span className="fs-22 fw-bold text-dark d-block leading-none">
                        {avgScore}
                      </span>
                      <span className="text-muted fs-10 text-uppercase tracking-wider">
                        Score
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p
                      className={`fs-14 fw-bold mb-1 ${getScoreColorClass(avgScore)}`}
                    >
                      {getLighthouseLabel(avgScore)}
                    </p>
                    <p className="text-muted fs-12 mb-0">
                      Compiled from {summary?.scoredPages || 0} scanned pages
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="row g-3">
                    <div className="col-6 col-md-4">
                      <div className="p-3 bg-light rounded-3 text-center h-100 d-flex flex-column justify-content-between">
                        <div>
                          <span className="text-muted fs-11 d-block text-uppercase mb-1 fw-medium">
                            Loading Speed (LCP)
                          </span>
                          <p className="text-muted fs-10 mb-2 lh-sm">
                            How fast primary content loads on the screen
                          </p>
                          <span className="fs-18 fw-bold text-dark">
                            {formatVal(summary?.avgLCP, "s")}
                          </span>
                        </div>
                        <span
                          className={`badge rounded-pill d-block mt-2 py-1 ${getVitalsBadgeClass(summary?.avgLCP, "lcp")}`}
                        >
                          {summary?.avgLCP <= 2.5
                            ? "Good (< 2.5s)"
                            : summary?.avgLCP <= 4.0
                    ...
                    <div className="col-6 col-md-4">
                      <div className="p-3 bg-light rounded-3 text-center h-100 d-flex flex-column justify-content-between">
                        <div>
                          ...
                          <p className="text-muted fs-10 mb-2 lh-sm">
                            Delay on user clicks, taps, or keystrokes
                          </p>
                          <span className="fs-18 fw-bold text-dark">
                            {formatVal(summary?.avgINP, "ms")}
                          </span>
                        </div>
                        <span
                          className={`badge rounded-pill d-block mt-2 py-1 ${getVitalsBadgeClass(summary?.avgINP, "inp")}`}
                        >
                          {summary?.avgINP <= 200
                            ? "Good (< 200ms)"
                            : summary?.avgINP <= 500
                    ...
                    <div className="col-12 col-md-4">
                      <div className="p-3 bg-light rounded-3 text-center h-100 d-flex flex-column justify-content-between">
                        <div>
                          ...
                          <p className="text-muted fs-10 mb-2 lh-sm">
                            Unexpected layout shifts of page elements
                          </p>
                          <span className="fs-18 fw-bold text-dark">
                            {formatVal(summary?.avgCLS)}
                          </span>
                        </div>
                        <span
                          className={`badge rounded-pill d-block mt-2 py-1 ${getVitalsBadgeClass(summary?.avgCLS, "cls")}`}
                        >
                          {summary?.avgCLS <= 0.1
                            ? "Good (< 0.1)"
                            : summary?.avgCLS <= 0.25
              ...
              <div className="card border-0 shadow-sm bg-success bg-opacity-10 text-success border-start border-success border-4 h-100">
                <div className="card-body d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <p className="fs-13 text-success mb-0 opacity-75">
                      Score: 90 - 100
                    </p>
                  </div>
                  <span className="fs-28 fw-bold">
              ...
              <div className="card border-0 shadow-sm bg-warning bg-opacity-10 text-warning-emphasis border-start border-warning border-4 h-100">
                <div className="card-body d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <p className="fs-13 text-warning-emphasis mb-0 opacity-75">
                      Score: 50 - 89
                    </p>
                  </div>
                  <span className="fs-28 fw-bold">
                    {summary?.distribution?.needsImprovement || 0}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="card border-0 shadow-sm bg-danger bg-opacity-10 text-danger border-start border-danger border-4 h-100">
                <div className="card-body d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <p className="fs-13 text-danger mb-0 opacity-75">
                      Score: 0 - 49
                    </p>
                  </div>
                  <span className="fs-28 fw-bold">
            ...
            <div className="card-body p-0">
              ...
              <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 p-4 border-bottom border-secondary border-opacity-10">
                <div className="d-flex align-items-center gap-2">
                  ...
                  <span className="text-muted fs-13 flex-shrink-0">
                    entries
                  </span>
                </div>
              ...
              {loading ? (
                <div className="text-center py-5">
                  ...
                  <p className="mt-2 text-muted fs-13">
                    Fetching performance pages...
                  </p>
                </div>
              ) : pages.length === 0 ? (
                <div className="text-center text-muted py-5">
                  No pages found or match your search criteria.
                </div>
              ) : (
                    ...
                    <tbody>
                      {pages.map((p, i) => (
                        <tr key={p.id || i}>
                          <td className="ps-4">
                            <h6
                              className="mb-1 fw-semibold text-body fs-13 text-truncate"
                              style={{ maxWidth: 380 }}
                              title={p.title}
                            >
                              {p.title}
                          ...
                          <td className="text-center">
                            <span
                              className={`badge py-2 px-3 rounded-pill fs-12 ${getScoreBgClass(p.performanceScore)}`}
                            >
                              {p.performanceScore}
                            </span>
                          </td>
                          <td className="text-center">
                            <span
                              className={`badge py-2 px-3 rounded-pill fs-12 ${getVitalsBadgeClass(p.lcp, "lcp")}`}
                            >
                              {formatVal(p.lcp, "s")}
                            </span>
                          </td>
                          <td className="text-center">
                            <span
                              className={`badge py-2 px-3 rounded-pill fs-12 ${getVitalsBadgeClass(p.inp, "inp")}`}
                            >
                              {formatVal(p.inp, "ms")}
                            </span>
                          </td>
                          <td className="text-center">
                            <span
                              className={`badge py-2 px-3 rounded-pill fs-12 ${getVitalsBadgeClass(p.cls, "cls")}`}
                            >
                              {formatVal(p.cls)}
                            </span>
                          </td>
