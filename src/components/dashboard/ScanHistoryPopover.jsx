...
export default function ScanHistoryPopover({ latestSummary }) {
  ...
  const lastScanDate = latestSummary?.lastScanDate 
    ? new Date(latestSummary.lastScanDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    : "N/A";

  const SCAN_METRICS = [
    { color: "#ef4444", label: "High Priority", value: latestSummary?.issueBreakdown?.high || 0 },
    { color: "#f59e0b", label: "Medium Priority", value: latestSummary?.issueBreakdown?.medium || 0 },
    { color: "#3b82f6", label: "Low Priority", value: latestSummary?.issueBreakdown?.low || 0 },
    { color: "#10b981", label: "SEO Score", value: (latestSummary?.finalSeoScore || 0) + "%" },
          ...
          {SCAN_METRICS.map(({ color, label, value }) => (
            <li key={label} className="d-flex align-items-center gap-2 mb-2">
              ...
              <span className="text-body" style={{ fontSize: '12px' }}>
                {label}: <span className="fw-semibold text-primary">{value}</span>
              </span>
            </li>
          ))}
        </ul>
