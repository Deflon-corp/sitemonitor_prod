...
export default function DashboardCharts({ historyData = [] }) {
  ...
  useEffect(() => {
    ...
    const initCharts = () => {
      const ApexCharts =
        typeof window !== "undefined" ? window.ApexCharts : undefined;
      ...
      if (scanHistoryEl) {
          ...
          const items = [
            {
              color: "#F38BBB",
              label: "Policies",
              value: h.complianceSummary?.termsFound ? "100%" : "0%",
            },
            {
              color: "#5297FE",
              label: "QA",
              value: (h.issueBreakdown?.high || 0) + " issues",
            },
            {
              color: "#7539FF",
              label: "Accessibility",
              value: (h.performanceMetrics?.avgAccessibilityScore || 0) + "%",
            },
            {
              color: "#c4956a",
              label: "SEO",
              value: (h.finalSeoScore || 0) + "%",
            },
            {
              color: "#00D4FF",
              label: "Pages crawled",
              value: (h.totalPages || 0) + " pages",
            },
            {
              color: "#94a3b8",
          ...
          series: [
            {
              name: "QA Issues",
              type: "column",
              data: history.map((h) => h.issueBreakdown?.high || 0),
            },
            {
              name: "Accessibility",
              type: "column",
              data: history.map(
                (h) => h.performanceMetrics?.avgAccessibilityScore || 0,
              ),
            },
            {
              name: "SEO",
              type: "column",
              data: history.map((h) => h.finalSeoScore || 0),
            },
            {
              name: "Total Crawled",
              type: "line",
              data: history.map((h) => h.totalPages || 0),
            },
          ],
          stroke: {
            width: [0, 0, 0, 3],
        ...
        scanHistoryChartRef.current = new ApexCharts(
          scanHistoryEl,
          scanHistoryOptions,
        );
