...
const Dashboard = () => {
  ...
  const calculateDashArray = (percentage) => {
    ...
    const value =
      (Math.min(100, Math.max(0, percentage || 0)) / 100) * circumference;
  ...
  const getUptimeStatus = () => {
    if (!latestSummary)
      return {
        ...
        className:
          domainData?.dm_status === "active" ? "text-success" : "text-danger",
        icon:
          domainData?.dm_status === "active"
            ? "isax-tick-circle"
            : "isax-close-circle",
      };

    const isUp =
      latestSummary.rootHttpStatus >= 200 && latestSummary.rootHttpStatus < 400;
    return {
      text: isUp ? "OK" : "Down",
  ...
  const getLastDowntime = () => {
    ...
    const failures = scanHistory.filter(
      (h) => h.rootHttpStatus < 200 || h.rootHttpStatus >= 400,
    );
                ...
                <>
                  ...
                  <div className="d-flex align-items-center flex-wrap gap-4 mt-2">
                    <div className="d-flex align-items-center fs-13 text-white">
                      ...
                      <span className="me-2">
                        Last Scan:{" "}
                        {domainData.dm_last_scan_at
                          ? `${formatDate(domainData.dm_last_scan_at)} ${formatTime(domainData.dm_last_scan_at)}`
                          : "Never"}
                      </span>
