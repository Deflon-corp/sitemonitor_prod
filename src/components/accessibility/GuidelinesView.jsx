...
const PRINCIPLES = [
  {
    ...
    guidelines: [
      ...
      {
        id: "1.2",
        title: "Time-based Media: Provide alternatives for time-based media.",
        level: null,
        error: 0,
        warning: 0,
        review: 0,
  ...
  },
  {
    ...
    guidelines: [
      {
        id: "2.1",
        title: "Keyboard: Make all functionality available from a keyboard.",
        level: null,
        error: 0,
        warning: 0,
        review: 0,
  ...
  },
  {
    ...
    guidelines: [
      {
        id: "3.1",
        title: "Readable: Make text content readable and understandable.",
        level: null,
        error: 0,
        warning: 0,
        review: 0,
  ...
  const principlesWithGroups = useMemo(
    () =>
      filteredPrinciples.map((p) => ({
        ...p,
        groups: groupGuidelines(p.guidelines),
      })),
    [filteredPrinciples],
  );
  ...
  return (
    <div className="guidelines-view">
      ...
      <div className="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-4">
        <div>
          <h5 className="mb-1 fw-semibold text-body d-flex align-items-center gap-2">
            <i
              className="isax isax-menu text-primary fs-22"
              aria-hidden="true"
            />{" "}
            Guidelines for WCAG 2.2
          </h5>
          <p className="text-muted fs-13 mb-0">
            List of checks with and without issues categorized by WCAG 2.2
            guidelines
          </p>
        </div>
        <div
              ...
              <thead>
                <tr className="border-bottom border-secondary border-opacity-25 bg-body-tertiary bg-opacity-50">
                  ...
                  <th className="py-3 px-3 text-body fs-13 fw-semibold text-nowrap text-end">
                    ...
                    <i
                      className="isax isax-arrow-down-1 fs-12 opacity-50 ms-1"
                      aria-hidden="true"
                    />
                  </th>
                  <th className="py-3 px-3 text-body fs-13 fw-semibold text-nowrap text-end">
                ...
                {principlesWithGroups.map((principle) => (
                  <React.Fragment key={principle.id}>
                    <tr
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && togglePrinciple(principle.id)
                      }
                    >
                      <td className="py-3 ps-4" colSpan={4}>
                        <div className="d-flex align-items-center gap-2">
                          <i
                            ...
                            style={{
                              transform: expandedPrincipleIds.has(principle.id)
                                ? undefined
                                : "rotate(-90deg)",
                            }}
                            aria-hidden="true"
                      ...
                      principle.groups.map(({ parent, children }) => (
                        <React.Fragment key={`${principle.id}-${parent.id}`}>
                          <tr
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && toggleGuideline(parent.id)
                            }
                          >
                            <td className="py-2 ps-4" colSpan={4}>
                              <div className="d-flex align-items-center gap-2">
                                <i
                                  ...
                                  style={{
                                    transform: expandedGuidelineIds.has(
                                      parent.id,
                                    )
                                      ? undefined
                                      : "rotate(-90deg)",
                                ...
                                />
                                <i
                                  className="isax isax-document-text text-muted fs-16 flex-shrink-0"
                                  aria-hidden="true"
                                />
                              ...
                              <tr key={`${principle.id}-${item.id}`}>
                                ...
                                <td className="py-2 px-3 text-end">
                                  <button
                                    ...
                                    onClick={() =>
                                      openIssuesDrawer(item, "error")
                                    }
                                    aria-label={`${item.error} errors – open list`}
                                  >
                                    <i
                                      className="isax isax-danger text-danger fs-14"
                                      aria-hidden="true"
                                    />
                                ...
                                <td className="py-2 px-3 text-end">
                                  <button
                                    ...
                                    onClick={() =>
                                      openIssuesDrawer(item, "warning")
                                    }
                                    aria-label={`${item.warning} warnings – open list`}
                                  >
                                    <i
                                      className="isax isax-notification text-warning fs-14"
                                      aria-hidden="true"
                                    />
                                ...
                                <td className="py-2 pe-4 text-end">
                                  <button
                                    ...
                                    onClick={() =>
                                      openIssuesDrawer(item, "review")
                                    }
                                    aria-label={`${item.review} review – open list`}
                                  >
                                    <i
                                      className="isax isax-eye text-primary fs-14"
                                      aria-hidden="true"
                                    />
      ...
      <AccessibilityIssuesDrawer
        ...
        guidelineLabel={
          issuesDrawerContext
            ? `${issuesDrawerContext.guidelineId} ${issuesDrawerContext.guidelineTitle}`
            : undefined
        }
