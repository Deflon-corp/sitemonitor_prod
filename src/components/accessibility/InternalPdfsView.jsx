...
const PdfTrendChart = ({ labels = ["Dec 09", "Jan", "Feb 15"], reviewedData = [], pendingData = [] }) => {
    ...
    import("@/api/inventoryApi").then(({ default: inventoryApi }) => {
      ...
      inventoryApi.getInventoryDetails(domainId, { type: "documents", limit: 500 }).then(res => {
        if (res.success && res.data) {
          ...
          const internalDocs = allDocs.filter(d => {
            try {
    ...
    <div className="internal-pdfs-view">
        ...
        <div>
          <h5 className="mb-1 fw-semibold text-body d-flex align-items-center gap-2">
            <div className="rounded-circle bg-primary bg-opacity-10 d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: 40, height: 40 }}>
              <i className="isax isax-document-text text-primary fs-20" aria-hidden="true" />
            </div>
            Internal PDF Documents
          </h5>
          <p className="text-muted fs-14 mb-0 ms-5" style={{ paddingLeft: '8px' }}>
            A comprehensive list of all PDF documents hosted on your domain that were discovered during the website scan.
          </p>
        </div>
      ...
      <div className="mb-4">
        {chartData.labels.length > 0 && (
          <PdfTrendChart labels={chartData.labels} reviewedData={chartData.reviewed} pendingData={chartData.pending} />
        )}
      </div>
